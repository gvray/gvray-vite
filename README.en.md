...
🦄 An enterprise-grade admin system built with **React 19**, **Vite**, **Ant Design 6**, and **TypeScript**, focused on **modern frontend architecture** and an **RBAC permission system**. It provides config-based routing, multi-theme support, internationalization, runtime config, Mock, and OpenAPI out of the box, ready to serve as a Starter Template for enterprise admin projects.
...
| 🧭 Config-based Routing | React Router 8 with a route table, lazy loading, and permission guards |
| 🎨 Multi-Theme | Light, dark, color-weak, and grayscale switching via antd 6 native cssVar injection |
...
| 🎭 Mock | vite-plugin-mock for seamless switching between Mock and real APIs |
...
| ⚡ React Compiler | React Compiler enabled for automatic render optimization, reducing manual memo |
...
```bash
# Dev environment (with built-in Mock data), port 9527
pnpm start:dev

# Staging environment (connected to gvray-admin backend), port 9528 — recommended
pnpm start:staging

# Production config, port 9529
pnpm start:prod
```
...
| --- | --- | --- |
| Mock (`pnpm start:dev`) | `admin` | `123456` |

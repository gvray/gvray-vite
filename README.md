...
🦄 基于 **React 19**、**Vite**、**Ant Design 6**、**TypeScript** 构建的企业级后台管理系统，专注于 **现代前端架构** 与 **RBAC 权限体系** 设计，提供配置式路由、多主题、国际化、运行时配置、Mock、OpenAPI 等核心能力，可直接作为企业后台项目的 Starter Template。
...
| 🧭 配置式路由 | 基于 React Router 8，路由表 + 懒加载 + 权限守卫 |
| 🎨 多主题 | 支持浅色、深色、色弱、灰度等主题切换，antd 6 原生 cssVar 注入 |
...
| 🎭 Mock | vite-plugin-mock，Mock 与真实接口无缝切换 |
| 📄 OpenAPI | 自动同步接口定义与 TypeScript 类型 |
| 🏗️ TablePro 增强表格 | 内置高级搜索、分页、刷新、useTablePro Hook |
| ⚡ React Compiler | 启用 React Compiler 自动优化渲染，减少手动 memo |
...
```bash
# 开发环境（内置 Mock 数据），端口 9527
pnpm start:dev

# 测试环境（连接 gvray-admin 后端），端口 9528 — 推荐
pnpm start:staging

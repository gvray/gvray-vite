...
export DOCKER_NAMESPACE=gvray

# 版本配置
export VERSION=1.0.0

# 容器配置
export CONTAINER_NAME=gvray-vite-web
...
# 查看最近 100 行日志
docker logs --tail=100 gvray-vite-web
...
```bash
# 进入容器 shell
docker exec -it gvray-vite-web sh
```
...
# 或手动扫描
trivy image gvray/gvray-vite:latest
```
...
```bash
# 查看容器日志
docker logs gvray-vite-web

# 检查容器状态
docker inspect gvray-vite-web

# 查看健康检查状态
docker inspect --format='{{.State.Health.Status}}' gvray-vite-web
```
...
# 修改端口映射
docker run -p 8888:80 gvray/gvray-vite:latest
```

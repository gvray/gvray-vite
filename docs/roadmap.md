...
| 功能 | 优先级 | 状态 | 说明 | 可落地操作 |
| --- | --- | --- | --- | --- |
| 路由加载进度条（NProgress） | 高 | ✅ | 页面切换时顶部显示进度条 | `lazyProgress` 包裹懒加载路由，驱动 NavigationProgress |
...
| 主题管理 | 中 | ✅ | 暗黑 / 浅色 / 自定义主题 + 色弱 / 灰度模式 | antd 6 ConfigProvider cssVar + 自建主题系统 |
...
| 功能 | 优先级 | 状态 | 说明 | 可落地操作 |
| --- | --- | --- | --- | --- |
| UI 组件库集成 | 高 | ✅ | Ant Design 6 | 安装组件库 + ConfigProvider + cssVar |
| 样式管理 | 高 | ✅ | SCSS / CSS 变量 / styled-components | 统一样式方案，支持主题变量和隔离 |
| 微前端架构 | 中 | ⬜ | Qiankun / single-spa 集成 | 主子应用独立构建 + 子应用注册 |
| 开发代理 | 高 | ✅ | 跨域请求 / Mock | Vite devServer proxy + vite-plugin-mock |
| OpenAPI 类型自动同步 | 中 | ✅ | API 类型同步到前端 | `pnpm gen:api`（OpenAPI → TypeScript） |
...
| 本地开发热更新 | 高 | ✅ | HMR 提升开发体验 | Vite HMR |
| React Compiler | 中 | ✅ | 自动渲染优化，减少手动 memo | babel-plugin-react-compiler |
...
| 功能 | 优先级 | 状态 | 说明 | 可落地操作 |
| --- | --- | --- | --- | --- |
| 多环境构建 & .env | 高 | ✅ | dev / staging / prod 配置 | Vite 环境变量配置（env.ts + load-env.ts） |
| TypeScript 全面支持 | 高 | ✅ | 类型安全开发 | tsconfig.json 配置 + 类型检查 |
| ESLint | 高 | ✅ | 统一代码规范 | eslint.config.js（flat config）+ typescript-eslint |
| Git 提交钩子 | 中 | ⬜ | commitlint / husky / lint-staged | 配置 husky + commitlint 强制 commit 格式 |
| 单元测试 / 集成测试 | 中 | ⬜ | Vitest | 编写测试用例 + CI 流程 |
| 构建优化 | 高 | 🟡 | Tree Shaking / Code Splitting / Lazy Loading | Vite 构建配置 + 路由懒加载 |

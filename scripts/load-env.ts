...
export const loadEnvFromEnv = (mode: string, opts: { port?: number | string; mock?: string } = {}) => {
  ...
  console.log(`  ${c.dim}➜${c.reset}  ${c.bold}mode${c.reset} ${c.gray}:${c.reset} ${mode}      ${c.dim}➜${c.reset}  ${c.bold}port${c.reset} ${c.gray}:${c.reset} ${port ?? '-'}      ${c.dim}➜${c.reset}  ${c.bold}mock${c.reset} ${c.gray}:${c.reset} ${mockOn ? `${c.green}on${c.reset}` : `${c.gray}off${c.reset}`}\n`);

  const rows = ENV_KEYS
    .map((k) => [k, process.env[k] as string | undefined])
    .filter(([, v]) => v !== undefined && v !== '') as [string, string][];

  const overriddenCount = rows.filter(([k]) => overridden.has(k)).length;
  console.log(`  ${c.dim}Environment${c.reset} ${c.gray}(${rows.length})${c.reset}${overriddenCount > 0 ? `  ${c.yellow}${overriddenCount} overridden by .env${c.reset}` : ''}`);
  for (const [k, v] of rows) {
    ...
    console.log(`  ${c.gray}•${c.reset} ${c.bold}${padRight(k, 24)}${c.reset} ${valColor}${v}${valReset}${sourceTag}`);
  }
  console.log('');
};
